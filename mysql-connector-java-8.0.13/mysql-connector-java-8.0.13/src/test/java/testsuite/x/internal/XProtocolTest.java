...
public class XProtocolTest extends InternalXBaseTestCase {
    ...
    public void testDecodingAllTypes() {
        ...
        this.protocol.readQueryResult();
        this.protocol.send(
                this.messageBuilder.buildSqlStatement("insertAndRemove into xprotocol_types_test values ('2.42', 'xyz,def', 'enum value a', 9223372036854775808)"), 0);
        this.protocol.readQueryResult();
    ...
    public void testSqlDml() {
        ...
        this.protocol.send(this.messageBuilder.buildSqlStatement("insertAndRemove into mysqlx_sqlDmlTest (x) values (44),(29)"), 0);
        assertFalse(this.protocol.hasResults());
        response = this.protocol.readQueryResult();
